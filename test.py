            ...
            try:
                if base64_data:
                    ...
                    try:
                        ...
                        raise HTTPException(
                            status_code=400, 
                            detail=f"Invalid base64 data for {face_name}: {str(e)}"
                        )
                    ...
                    try:
                        ...
                        raise HTTPException(
                            status_code=400, 
                            detail=f"Invalid image data for {face_name}: {str(e)}"
                        )
                    ...
                    try:
                        ...
                        raise HTTPException(
                            status_code=500, 
                            detail=f"Failed to upload {face_name} to MinIO: {str(e)}"
                        )
                ...
                raise HTTPException(
                    status_code=500,
                    detail=f"Error processing {face_name}: {str(e)}"
                )

        # Process fingerprint images with similar error handling...
        # [Rest of the fingerprint processing code remains the same] 
